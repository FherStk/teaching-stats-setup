...
setup_gauth()
{
  ...
  if ! [ -f "$MARK" ]; then             
    ...
    echo
    runuser -l postgres -c "psql -d \"${BBDD}\" -e -c 'INSERT INTO public.socialaccount_socialapp (id, provider_id, provider, name, client_id, secret, key, settings) VALUES (1, 1, '\'google\'', '\'google-api\'', '\'${CLIENT}\'', '\'${SECRET}\'', '\'\'', '\{}'\'');'"
