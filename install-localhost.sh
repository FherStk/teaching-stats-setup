...
setup_gauth(){
  ...
  if ! [ -f "$MARK" ]; then    
    ...
    echo -e "        App name: ${CYAN}${BBDD}${NC}"
    echo -e "        Support email: ${CYAN}${EMAIL}${NC}"
    echo -e "        Developer contact information: ${CYAN}${EMAIL}${NC}"
    echo -e "    6. Save and continue."
    echo -e "    7. Set  it up:"
    echo -e "        Application type: ${CYAN}Web application${NC}"
    echo -e "        Name: ${CYAN}${BBDD}${NC}"
    echo -e "        Authorized JavaScript origins → Add URI: ${CYAN}${URL}${NC}"
    echo -e "        Authorized redirect URIs → Add URI: ${CYAN}${URL}/google/login/callback/${NC}"
    echo -e "    8. Copy your ${CYAN}client id${NC} and ${CYAN}secret key${NC}, it will be required later."
    echo ""
