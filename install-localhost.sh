...
metabase_bbdd()
{
  ...
  if ! [ -f "$MARK" ]; then          
    ...
    runuser -l postgres -c "psql -d \"${BBDD}\" -e -c 'CREATE EXTENSION IF NOT EXISTS citext WITH SCHEMA public; GRANT ALL ON ALL FUNCTIONS IN SCHEMA public TO \"${BBDD}\";ALTER USER \"${BBDD}\" SET current_schema=public'"    
